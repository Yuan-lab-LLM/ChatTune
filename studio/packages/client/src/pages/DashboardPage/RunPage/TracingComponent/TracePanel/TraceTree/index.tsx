...
const getDisplayKind = (attributes: Record<string, unknown>): string => {
    ...
    const medflow = attributes.medflow as
    ...
    const tool_name = (genAi?.tool as Record<string, unknown>)?.name as
        | string
        | undefined;
    const format_target = (medflow?.format as Record<string, unknown>)
...
export const TraceTree = ({ spans }: Props) => {
    ...
    const spanTitleDataMap = useMemo(() => {
        ...
        spans.forEach((span) => {
            const medflow = span.attributes.medflow as
                | Record<string, unknown>
                | undefined;
            const funcName = (medflow?.function as Record<string, unknown>)
