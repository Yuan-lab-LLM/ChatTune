...
export class SpanProcessor {
    ...
    public static decodeOTLPSpan(
        ...
        if (scope.name.toLowerCase().includes('medflow.tracing._trace')) {
    ...
    public static convertOldProtocolToNew(
        ...
        const medflow = newAttributes.medflow as Record<string, unknown>;
        ...
        const medflowFunction = medflow.function as Record<
            string,
            unknown
        >;

        medflowFunction.name = span.name;
        ...
        if (inputValue) medflowFunction.input = inputValue;
        ...
        const outputValue = getNestedValue(attributes, 'output') as
            ...
            | undefined;
        if (outputValue) {
            medflowFunction.output = outputValue;
