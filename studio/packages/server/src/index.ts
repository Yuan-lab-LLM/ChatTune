...
async function initializeServer() {
    try {
        ...
        managementRefreshScheduler.start();
        ...
        app.get('/api/version-check', async (_req, res) => {
            try {
                const response = await fetch(
                    'https://registry.npmjs.org/@medflow/studio',
                );
                if (!response.ok) {
                ...
                if (!latest) {
                    res.status(502).json({
                        error: 'Package information does not include a latest version',
                    });
                    return;
                }

                res.json({ 'dist-tags': { latest } });
            } catch (error) {
                console.warn('Failed to check npm package version:', error);
                res.status(502).json({
                    error: 'Failed to fetch package information',
                });
            }
        ...
        httpServer.listen(configManager.getConfig().port, configManager.getConfig().host, () => {
...
initializeServer()
    .then(({ httpServer, otelGrpcServer }) => {
        ...
        const cleanup = async () => {
            ...
            managementRefreshScheduler.stop();
