...
export class SpanDao {
    ...
    static async getTraceList(filters: {
        ...
        try {
            const queryBuilder = SpanTable.createQueryBuilder('span')
                .innerJoin(RunTable, 'run', 'run.id = span.conversationId')
            ...
            if (filters.ownerUserId) {
                queryBuilder.andWhere('run.ownerUserId = :ownerUserId', {
    ...
    static async getTrace(traceId: string, ownerUserId?: string): Promise<{
        traceId: string;
        spans: SpanData[];
        startTime: string;
        endTime: string;
        ...
        try {
            const spansQueryBuilder = SpanTable.createQueryBuilder('span')
                .innerJoin(RunTable, 'run', 'run.id = span.conversationId')
                .where('span.traceId = :traceId', { traceId })
                .orderBy('span.startTimeUnixNano', 'ASC');

            if (ownerUserId) {
                spansQueryBuilder.andWhere('run.ownerUserId = :ownerUserId', {
                    ownerUserId,
                });
            }

            const spans = await spansQueryBuilder.getMany();
